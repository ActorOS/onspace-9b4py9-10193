    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.content}>
          ...
          {/* Containment Tools - Contextual Somatic Releases */}
          {currentWorkloadLevel && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>Suggested Release for This Work</Text>
              </View>
              <Text style={styles.containmentHelperText}>
                Based on {currentWorkloadLevel} workload intensity
              </Text>
              {somaticExitStorage.getTracksForWorkload(currentWorkloadLevel).slice(0, 3).map(track => {
                const isLocked = !track.freeAccess && !isPro;
                return (
                  <Pressable
                    key={track.type}
                    style={[styles.releaseCard, isLocked && styles.releaseCardLocked]}
                    onPress={() => {
                      if (isLocked) {
                        setUpgradeFeature({
                          name: 'Full Somatic Exit Library',
                          description: 'Pro unlocks all professional cool-down tracks matched to workload intensity'
                        });
                        setShowUpgradePrompt(true);
                      } else {
                        router.push({
                  ...
                  >
                    ...
                    <View style={styles.releaseContent}>
                      <View style={styles.releaseHeader}>
                        <Text style={[styles.releaseTitle, isLocked && styles.releaseTitleLocked]}>
                          {track.title}
                        </Text>
                        {isLocked && (
                      ...
                      <Text style={[styles.releaseSubtitle, isLocked && styles.releaseSubtitleLocked]}>
                        {track.subtitle}
                      </Text>
                      <View style={styles.releaseMeta}>
                        <MaterialIcons name="schedule" size={14} color={isLocked ? colors.textTertiary : colors.textSecondary} />
                        <Text style={[styles.releaseDuration, isLocked && styles.releaseDurationLocked]}>
                          {track.duration}
                        </Text>
                      </View>
                    </View>
                    <MaterialIcons name="chevron-right" size={24} color={isLocked ? colors.border : colors.textTertiary} />
                  </Pressable>
                );
              })}
              <View style={styles.infoCard}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.guidanceHeadline}>Professional Recovery Tools</Text>
                  <Text style={styles.guidanceText}>
                    These release options match the demands of your work, supporting intentional exit and somatic containment.
                  </Text>
                </View>
              </View>
            </View>
