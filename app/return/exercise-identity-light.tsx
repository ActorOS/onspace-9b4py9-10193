...
export default function IdentitySeparationLightScreen() {
  ...
  const handleBegin = async () => {
    ...
    const tid = await trackExerciseStarted('Identity Separation (Light)');
  ...
  const handleComplete = async () => {
    ...
    if (trackingSessionId) {
      await trackExerciseCompleted('Identity Separation (Light)', trackingSessionId);
    }
  ...
  const handleExit = () => {
    ...
    const trackAbandon = async () => {
      if (hasStarted && currentStep !== 'complete' && trackingSessionId) {
        await trackExerciseAbandoned('Identity Separation (Light)', trackingSessionId);
      }
    };
