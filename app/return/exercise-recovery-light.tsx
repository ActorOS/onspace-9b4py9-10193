...
// Full Body Recovery Light - 5-minute condensed version
// Quick somatic release with guided breath - ~5 minutes
// Fully hands-free once started

type RecoveryLightStep = 
  | 'arrival' 
  | 'breathOrientation' 
  | 'breathCycle' 
  | 'faceRelease' 
...
export default function FullBodyRecoveryLightScreen() {
  ...
  const initSession = async () => {
    try {
      ...
      const session = await returnSessionStorage.saveExerciseSession({
        ...
        roleId,
        exerciseType: 'full_body_recovery_light',
        durationMinutes: 5,
        completed: false,
      });
  ...
  const runRecoveryLightSequence = async () => {
    try {
      // Step 1: Arrival
      ...
      console.log('Recovery light sequence failed, showing text fallback:', error);
      setAudioError(true);
      setCurrentStep('complete');
    }
  };
  ...
  const handleBegin = async () => {
    ...
    const tid = await trackExerciseStarted('Full Body Recovery (Light)');
  ...
  const handleComplete = async () => {
    ...
    if (trackingSessionId) {
      await trackExerciseCompleted('Full Body Recovery (Light)', trackingSessionId);
    }
  ...
  const handleExit = () => {
    ...
    const trackAbandon = async () => {
      if (hasStarted && currentStep !== 'complete' && trackingSessionId) {
        await trackExerciseAbandoned('Full Body Recovery (Light)', trackingSessionId);
      }
    };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Recovery — Light</Text>
          ...
          {!hasStarted ? (
            <>
              ...
              <Text style={styles.welcomeText}>
                ...
                {'\n\n'}
                Breath, release, scan, and reclaim.
              </Text>
            </>
          ) : (
            <>
