...
export default function FullBodyRecoveryStandardScreen() {
  ...
  const [currentStep, setCurrentStep] = useState<RecoveryStandardStep>('arrival');
  ...
  const initSession = async () => {
    try {
      ...
      const session = await returnSessionStorage.saveExerciseSession({
        ...
        roleId,
        exerciseType: 'full_body_recovery_standard',
        durationMinutes: 10,
        completed: false,
      });
  ...
  const runRecoveryStandardSequence = async () => {
    try {
      // Step 1: Arrival
      ...
      // Nervous System Reset Breath (3 cycles)
      setCurrentStep('resetBreath');
      for (let i = 1; i <= 3; i++) {
        startWaveAnimation();
      ...
      console.log('Recovery standard sequence failed, showing text fallback:', error);
      setAudioError(true);
      setCurrentStep('complete');
    }
  };
  ...
  const handleBegin = async () => {
    ...
    const tid = await trackExerciseStarted('Full Body Recovery (Standard)');
    setTrackingSessionId(tid);
    runRecoveryStandardSequence();
  };
  ...
  const handleComplete = async () => {
    ...
    if (trackingSessionId) {
      await trackExerciseCompleted('Full Body Recovery (Standard)', trackingSessionId);
    }
    ...
    try {
      ...
      if (!isStackMode) {
        ...
        await returnSessionStorage.saveReturnSession({
          ...
          notes: 'Full Body Recovery Standard completed',
        });
      }
  ...
  const handleExit = () => {
    ...
    const trackAbandon = async () => {
      if (hasStarted && currentStep !== 'complete' && trackingSessionId) {
        await trackExerciseAbandoned('Full Body Recovery (Standard)', trackingSessionId);
      }
    };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Recovery — Standard</Text>
